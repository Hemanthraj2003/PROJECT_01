...
projects.forEach(project => {
  ...
  if (!fs.existsSync(nodeModulesPath)) {
    console.log(`⚠️  ${project.name} dependencies not installed (node_modules missing)`);
    console.log(`   Run: cd ${project.path} && npm install${project.name === 'CarApp' ? ' --legacy-peer-deps' : ''}`);
    allGood = false;
    return;
  }
...
const envFiles = [
  ...
  { name: 'Root .env.example', path: './.env.example' }
];
...
envFiles.forEach(envFile => {
  if (fs.existsSync(envFile.path)) {
    ...
    console.log(`⚠️  ${envFile.name} not found`);
  }
});
...
if (allGood) {
  ...
  console.log('❌ Some issues were found. Please fix them before starting development.');
  console.log('');
